...
namespace Core {
    ...
    public :
        ...
        uint32_t Unlock()
        {
            ...
            if (_initialized != false) {
                ...
                // Similar to METROL-1198 SharedAccessGood
    ...
    class SharedAccessSignal : public SharedAccess<SharedAccessSignal> {
    public :
        SharedAccessSignal(const SharedAccessSignal&) = delete;
        SharedAccessSignal(SharedAccessSignal&&) = delete;
        SharedAccessSignal& operator=(const SharedAccessSignal&) = delete;
        SharedAccessSignal& operator=(SharedAccessSignal&&) = delete;

        SharedAccessSignal() = default;
        ~SharedAccessSignal() = default;
    ...
    class SharedAccessBroadcast : public SharedAccess<SharedAccessBroadcast> {
    public :
        SharedAccessBroadcast(const SharedAccessBroadcast&) = delete;
        SharedAccessBroadcast(SharedAccessBroadcast&&) = delete;
        SharedAccessBroadcast& operator=(const SharedAccessBroadcast&) = delete;
        SharedAccessBroadcast& operator=(SharedAccessBroadcast&&) = delete;

        SharedAccessBroadcast() = default;
        ~SharedAccessBroadcast() = default;
        ...
        uint32_t ReevaluateImplementation()
        {
            ...
            // 'Broadcast' is guarantueed to wake up all blocked threads /
            // processes
            if ((errval = pthread_cond_broadcast(&_condition)) == 0) {
            } else {
                _errval = errval;
                result = ::Thunder::Core::ERROR_GENERAL;
            }
    ...
    TEST(METROL_1199, Signal)
    {
        ...
        SharedAccessSignal sharedBuffer;

        ProcessEmulator<SharedAccessSignal> nonAgent{ ProcessEmulator<SharedAccessSignal>{ (N + 1) * workTime , waitTime, sharedBuffer } };

        std::array<ProcessEmulator<SharedAccessSignal>, N> agents = { ProcessEmulator<SharedAccessSignal>{ workTime, waitTime, sharedBuffer }, ProcessEmulator<SharedAccessSignal>{ workTime, waitTime, sharedBuffer }, ProcessEmulator<SharedAccessSignal>{ workTime, waitTime, sharedBuffer } };
        ...
        while (   std::all_of
                  (
                     agents.begin()
                   , agents.end()
                   , [&workTime](ProcessEmulator<SharedAccessSignal>& agent) { return agent.Wait(::Thunder::Core::Thread::STOPPED | ::Thunder::Core::Thread::STOPPING | ::Thunder::Core::Thread::BLOCKED, workTime) != false; }
                  ) != true
    ...
    TEST(METROL_1199, Broadcast)
    {
        ...
        SharedAccessBroadcast sharedBuffer;

        ProcessEmulator<SharedAccessBroadcast> nonAgent{ ProcessEmulator<SharedAccessBroadcast>{ (N + 1) * workTime, waitTime , sharedBuffer } };

        std::array<ProcessEmulator<SharedAccessBroadcast>, N> agents = { ProcessEmulator<SharedAccessBroadcast>{ workTime, waitTime, sharedBuffer }, ProcessEmulator<SharedAccessBroadcast>{ workTime, waitTime, sharedBuffer }, ProcessEmulator<SharedAccessBroadcast>{ workTime, waitTime, sharedBuffer } };
        ...
        while (   std::all_of
                  (
                     agents.begin()
                   , agents.end()
                   , [&workTime](ProcessEmulator<SharedAccessBroadcast>& agent) { return agent.Wait(::Thunder::Core::Thread::STOPPED | ::Thunder::Core::Thread::STOPPING | ::Thunder::Core::Thread::BLOCKED, workTime) != false; }
                  ) != true
               && !(timedOut = ((stopWatch.Elapsed() / 1000) > ((N + 1) * workTime))) != false
        ) {}
