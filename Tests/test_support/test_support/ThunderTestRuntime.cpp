...
namespace TestCore {
        ...
        JsonObject& response)
    {
        ...
        params.ToString(serializedParams);

        string serializedResponse;
        const uint32_t result = Invoke(method, serializedParams, serializedResponse);

        if (serializedResponse.empty() == false) {
            response.FromString(serializedResponse);
        }

        return result;
    }

    uint32_t ThunderTestRuntime::JSONRPCLink::Subscribe(const string& event, EventHandler handler, const string& index)
    {
        uint32_t result = Core::ERROR_UNAVAILABLE;
        PluginHost::IDispatcher* dispatcher = Dispatcher();

        if (dispatcher != nullptr) {
            result = dispatcher->Subscribe(this, event, _callsign, index);
    ...
    uint32_t ThunderTestRuntime::JSONRPCLink::Unsubscribe(const string& event, const string& index)
    {
        uint32_t result = Core::ERROR_UNAVAILABLE;
        PluginHost::IDispatcher* dispatcher = Dispatcher();
        ...
        if (dispatcher != nullptr) {
            ...
            result = dispatcher->Unsubscribe(this, event, _callsign, index);
    ...
    ThunderTestRuntime::JSONRPCLink* ThunderTestRuntime::QueryInterfaceByCallsign(const string& callsign)
    {
    ...
    uint32_t ThunderTestRuntime::Invoke(const string& method,
        const JsonObject& params, JsonObject& response)
    {
        string serializedParams;
        params.ToString(serializedParams);
    ...
    bool ThunderTestRuntime::Exists(PluginHost::IDispatcher* dispatcher,
