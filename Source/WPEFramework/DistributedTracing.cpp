...
namespace PluginHost {
    ...
    void DistributedTracing::Shutdown() {
...

        {
            ...
            for (auto& entry : _activeSpans) {
                if (entry.second.isParent) {
                    rdk_otlp_finish_distributed_trace();
                } else {
                    rdk_otlp_finish_child_span();
                }
                rdk_otlp_clear_trace_context(entry.second.contextKey.c_str());
            }
            _activeSpans.clear();
        }
        ...
        uint32_t channelId)
    {
        ...
        if (hasParent) {
            ...
            rdk_otlp_start_distributed_trace(ctxKey.c_str(), "jsonrpc.invoke");
        }
    ...
    void DistributedTracing::OnInvokeEnd(uint64_t spanId, uint32_t result) {
        ...
        if (meta.isParent) {
            rdk_otlp_finish_distributed_trace();
            rdk_otlp_clear_trace_context(meta.contextKey.c_str());
        } else {
            rdk_otlp_finish_child_span();
        }
    }
        ...
        const std::string& callsign, uint32_t interfaceId)
    {
        ...
        if (hasParent) {
            ...
            rdk_otlp_start_distributed_trace(ctxKey.c_str(), "comrpc.acquire");
        }
