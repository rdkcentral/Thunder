...
#ifdef THUNDER_DISTRIBUTED_TRACING
                        bool _traceActive = PluginHost::DistributedTracing::Instance().OnCOMRPCAcquireBegin(
                            "ExternalAccess", interfaceId);
#endif
                        result = _plugin->QueryInterface(interfaceId);
#ifdef THUNDER_DISTRIBUTED_TRACING
                        PluginHost::DistributedTracing::Instance().OnCOMRPCAcquireEnd(_traceActive, result != nullptr);
#endif
                    }
                    return result;
                }
                ...
                if ( (_jsonrpc == nullptr) || (IsActive() == false
                 ...
                 && IsHibernated() == false
                    ...
                    if ((result = _jsonrpc->Validate(token, method, message.Parameters.Value())) == Core::ERROR_PRIVILIGED_REQUEST) {
                        ...
                        string output;
#ifdef THUNDER_DISTRIBUTED_TRACING
                        bool _traceActive = PluginHost::DistributedTracing::Instance().OnInvokeBegin(
...
#ifdef THUNDER_DISTRIBUTED_TRACING
                        PluginHost::DistributedTracing::Instance().OnInvokeEnd(_traceActive, result);
#endif
