...
set(EXTENSION_PATH "${CMAKE_INSTALL_FULL_LIBDIR}/${NAMESPACE_LIB}/extensions" CACHE PATH "Extension path")
set(CONFIG_PATH "" CACHE PATH "Plugin configuration path")
set(EXTENSION_CONFIG_PATH "" CACHE PATH "Extension configuration path")
...
set(DIRECT_OUTPUT false CACHE STRING "Enable direct message output")
set(DIRECT_OUTPUT_ABBREVIATION false CACHE STRING "Use abbreviated direct message output")
set(DIRECT_OUTPUT_TIME true CACHE STRING "Include time in direct message output")
...
if(NOT CONFIG_PATH STREQUAL "")
    map_set(${CONFIG} configs ${CONFIG_PATH})
endif()
if(NOT EXTENSION_CONFIG_PATH STREQUAL "")
    map_set(${CONFIG} extensionconfigs ${EXTENSION_CONFIG_PATH})
endif()
...
if(DIRECT_OUTPUT)
    map_append(${MESSAGING_SETTINGS} output direct)
endif()
...
map()
    kv(abbreviated ${DIRECT_OUTPUT_ABBREVIATION})
    kv(time ${DIRECT_OUTPUT_TIME})
end()
ans(MESSAGING_DIRECT_SETTINGS)
...
ans(MESSAGING_TELEMETRY_SETTINGS)
...
if(MESSAGING)
    ...
    foreach(_entry IN LISTS MESSAGING_TYPE_DEFAULTS)
        string(REPLACE "=" ";" _default_parts "${_entry}")
        list(GET _default_parts 0 _type)
        list(GET _default_parts 1 _enabled)
        string(TOUPPER "${_type}" _type_upper)
        string(TOLOWER "${_type_upper}" _section_name)
        set(_section_variable "MESSAGING_${_type_upper}_SETTINGS")

        if(DEFINED ${_section_variable})
            set(_section_settings "${${_section_variable}}")
        else()
            map()
                key(settings)
            end()
            ans(_section_settings)
            set(${_section_variable} "${_section_settings}")
            map_append(${MESSAGING_SETTINGS} ${_section_name} ${_section_settings})
            map_append(${_section_settings} settings ___array___)
        endif()

        if(_enabled STREQUAL "ON")
            set(_enabled_value true)
        else()
            set(_enabled_value false)
        endif()

        map()
            kv(enabled ${_enabled_value})
        end()
        ans(_message_type_default)

        map_append(${_section_settings} settings ${_message_type_default})
    endforeach()
endif()
