...
option(ENABLE_TELEMETRY_ALL "Enable all telemetry categories (wildcard)" false)
...
if(FLUSH_LOGS)
    set(_FLUSH_LOGS true)
else()
    set(_FLUSH_LOGS false)
endif()

map()
    kv(path ${MESSAGECONTROL_PATH})
    kv(port ${MESSAGING_PORT})
    kv(flush ${_FLUSH_LOGS})
...
if(MESSAGING)
    ...
    if(ENABLE_TELEMETRY_ALL)
        set(_TELEMETRY_ENABLED true)
    else()
        set(_TELEMETRY_ENABLED false)
    endif()
    map()
        kv(category "AnyCategory")
        kv(enabled ${_TELEMETRY_ENABLED})
    end()
    ans(PLUGIN_ANY_CATEGORY_TELEMETRY)
    ...
    map_append(${MESSAGING_TELEMETRY_SETTINGS} settings ${PLUGIN_ANY_CATEGORY_TELEMETRY})
endif()
