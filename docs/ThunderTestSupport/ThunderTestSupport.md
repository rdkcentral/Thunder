...
| Member | Description |
|--------|-------------|
| `Invoke()` | Invoke a method by bare name (e.g. `"echo"`), without repeating the callsign. |
...
# Build the test executable — include one dedicated Module.cpp
...
TEST_F(MyPluginTest, JSONRPC_OperationTriggersEvent)
{
    ...
    // Subscribe before calling the operation that internally fires the event.
    ...
    // Invoking this method causes the plugin to fire onStateChanged internally.
    string response;
...
public:
    ...
    // Block until at least expectedCount notifications have arrived.
...
TEST_F(MyPluginTest, COMRPC_OperationTriggersNotification)
{
    ...
    // PerformAction() does its work AND internally fires OnStateChanged on all sinks.
...
// Multiple operations each fire one notification — wait for all of them.
...
`JSONRPCLink` implements `IDispatcher::ICallback`. Thunder's dispatcher calls `AddRef()` / `Release()` on the callback when subscribing/unsubscribing (see `JSONRPC.h` `Destination` class). The callback therefore uses a real atomic ref-count with `delete this` on zero. Always call `Release()` when you are done with a `JSONRPCLink`, never `delete` it directly.
