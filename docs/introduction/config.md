...
| postmortempath                    | Directory to store debugging info (worker pool information, minidumps) in the event of a plugin or server crash | string    | /opt/minidumps                                               | /opt/minidumps                                        |
...
| signature                         |                                                              | string    | TestSecretKey                                                | TestSecretKey                                         |
...
| ipv6                              | Enable/disable IPv6 support                                  | bool      | false                                                        | true                                                  |
| legacyinitalize                   | Enables legacy Plugin initialization behaviour where the Deinitialize() method is not called on if Initialize() fails | bool      | false                                                        | false                                                 |
...
| input.locator                     | Socket to receive key events over                            | string    | /tmp/keyhandler\|0766                                        | -                                                     |
| input.type                        | Input device type (either `device` or `virtual`)             | string    | Virtual                                                      | Device                                                |
| input.output                      | Whether input events should be re-output for forwarding      | bool      | true                                                         | -                                                     |
...
| ethernetcard                      | Using the MAC address of this interface, Thunder will generate a unique identifier | string    | -                                                            | `eth0`                                                |
| plugins                           | Array of plugin configurations. Not recommended  - each plugin should have its own config file. Normally only used for Controller plugin configuration | array     | -                                                            | -                                                     |
| environments                      | Array of environment variables to set for the WPEFramework process. Each item in the array should be an object with `key`, `value`, and `override` properties. | array     | -                                                            | `[{"name": "FOO", "value": "BAR", "override": true}]` |
...
| messagingport                     | By default, the messaging engine sends log/trace messages over a unix socket. Provide a TCP port here to use that port instead | int       | -                                                            | 3000                                                  |
