...
The index tag can also be allowed to indicate an event uses an index parameter so clients can subscribe to an event only for a certain value of that index: for more info see [here](../interfaces/#indexed-events)

As for a short amount of time the @index for events used a dot (.) as a separator in the clientid instead of a @ in the designator, @index:deprecated can be put at the indexed event parameter to indicate this one should use the old deprecated index format (see the generated documentation for what the interface would be like but only use this for backwards compatibility reasons). 
...
Incorrect or inconsistent usage will lead to an error to be generated by the code generators. Of course the documentation generators do take the wrapped tag into account.
